# Econometria Avançada A8 - Diagnósticos de Resíduo
...
variacao_PIB <- read.table("C:/Econometria/variacao.xls", header = T)  #Lê o arquivo variacao.xls na pasta c:/Econometria #PCNERES
var_PIB <- ts(variacao_PIB$variacao_PIB, start =1951, frequency = 1 )  #Cria a serie temporal var_PIB

#Código para aplicar o teste de normalidade dos resíduos, nos dados informados

AR2 <- arima(var_PIB,c(2,0,0))        #Estima um modelo AR2                      
residuosAR2 <- AR2$residuals          #Extrai os resíduos do modelo AR2

# Criando o Histograma dos Resíduos (Grafico de Barra ótimo)
